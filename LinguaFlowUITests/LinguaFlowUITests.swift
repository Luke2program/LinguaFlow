...
final class LinguaFlowUITests: XCTestCase {
    ...
    func testSubjectSwitchButtonExists() throws {
        ...
        XCTAssertTrue(element("questRouletteTrainingPlanStrip", in: app).exists)
        XCTAssertTrue(element("questRouletteTrainingPlanTitle", in: app).label.contains("Today's Plan"))
        XCTAssertTrue(element("questRouletteTrainingPlanProgress", in: app).label.contains("live routes"))
        XCTAssertTrue(element("questRouletteTrainingPlanCard_best-run", in: app).exists)
        XCTAssertTrue(element("questRouletteTrainingPlanCard_cross-train", in: app).exists)
        XCTAssertTrue(element("questRouletteTrainingPlanCard_world-tour", in: app).exists)
